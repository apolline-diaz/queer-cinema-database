...
export default function MultiSelect({
  ...
  return (
    <div className="text-rose-600 col-span-2 relative">
        ...
        render={({ field }) => {
          ...
          return (
            <div className="relative">
              ...
              <input
                type="text"
                className="block appearance-none text-black placeholder-gray-500 w-full text-sm font-light p-2 border border-rose-600  rounded-md bg-white"
              ...
              {inputValue && filteredOptions.length > 0 && (
                <ul className="left-0 right-0 bg-bred-100 rounded-md border border-rose-600 text-sm font-light mt-1 z-10 max-h-40 overflow-y-auto">
                  {filteredOptions.map((option) => (
                    <li
                      key={option.value}
                      className="px-4 py-2 cursor-pointer hover:text-white hover:bg-rose-700"
                      onClick={() => {
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {safeValue.map((option: Option) => (
                  <span
                    key={option.value}
                    className="inline-flex items-center bg-rose-50 border border-rose-600  text-rose-600 text-sm  font-light px-2 py-1 rounded"
                  >
                    {option.label}
                    <button
                      type="button"
