  ...
  const onSubmit = async (data: ForgotPasswordFormInputs) => {
    try {
      ...
      const { error: resetError } = await supabase.auth.resetPasswordForEmail(
        data.email,
        {
          redirectTo: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/account/reset-password?access_token={access_token}`,
        }
      );
        ...
        {isEmailSent ? (
          <div className="flex flex-col gap-3">
            ...
            <div className="mt-6 w-full justify-center items-center flex">
              <Link
                href="/login"
                className="rounded-md border px-6 py-2 border-rose-600 hover:bg-rose-700 hover:text-white transition-colors"
              >
